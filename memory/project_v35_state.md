...
**Client deck (`client_alignment_deck.pptx`):** 12 slides at last update, generated by `build_client_deck.py` using python-pptx native shapes (boxes, decision diamonds, connectors, tables — not ASCII art, not pandoc-rendered). 16:9 widescreen, Calibri throughout, footer at `y=7.18` with deck label + slide number. Slide order:
...
6. Phase 2 — Per-SOP (flowchart inside `ForEach SOP` dashed container + Story YAML + 4 quality-rules cards)
7. Phase 3 — Combine (synthesis flowchart + capability story YAML + ≥ 2-SOPs lift visual)
8. The 4 story shapes — one example each (2×2 card grid)
9. Inside two key agents — Validator workflow + Dependency Resolver DAG
10. Validator at work — sample story walks through the checks (attempt 1 fails → revise → attempt 2 passes)
11. What happens when something fails (3-level fallback + 3-queues banner)
12. What ships and the alignment ask (3 deliverables + mandatory/optional client inputs)
