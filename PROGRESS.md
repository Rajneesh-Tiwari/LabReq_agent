...
**Phase:** Architecture design — v3.5 (Generalization Reframe) is the current target. POC scope is now the *generalization mechanism* across clinical-lab disciplines, not single-discipline extraction. v3.5 deliverables shipped: `V3_5_SPEC.md` (formal spec, decisions D26–D35), `microbio_lims_architecture_v35.md` (clean canonical, ~900 lines, 8 schemas with field tables, two worked examples), `client_alignment_deck.pptx` (8-slide native PowerPoint deck for client alignment), `build_client_deck.py` (deck generator). Three rounds of 5-pass parallel QC have been completed. Next: client alignment meeting, then operational doc cleanup pass (PROGRESS / NEXT_STEPS / OPEN_QUESTIONS still carry pre-v3.5 single-discipline framing in places).

**Latest framing (v3.5):** Generalization reframe layered on v3.4. Three lifts: (1) Theme Discovery runs for every new discipline (no per-discipline deferral); (2) universal 6-stage workflow enum (`lab_stage_v1`) added as a closed-enum field on Stories; (3) `actor_type` discriminator on persona-catalog entries (human / system / external_system). Plus two simplifications: (a) D34 — prior discipline's role narrowed to **theme catalog + epic catalog (mandatory)** + story exemplars (optional); holdout calibration dropped (Validator runs on default thresholds tuned by first-run telemetry); tasks unused. (b) D35 — intake scope-check removed; every SOP enters the pipeline. Plus quorum tuning: D33 lowered the vote threshold from M=4 to M=3 (simple majority of N=5); description-cosine convergence stays strict at min-pairwise ≥ 0.8. Plus mechanism extension: D32 added Pass 3b discard quorum for sparsely-classified inherited themes.

**Earlier framing (v3.2, preserved as foundation):** Conditioned Discovery — warm-start (don't cold-start) at both theme and epic levels. A pre-flight **Theme Discovery agent** classifies a new discipline's SOPs against the prior theme catalog (e.g. `cyto_v1`) at threshold `τ_match`, clusters only the residual at minimum size `ε_novelty`, and routes novel candidates through quorum (in v3.5) or SME (in v3.2). The **Epic Extractor runs in conditioned mode** — drafts are matched against the prior epic catalog (`cyto_epic_v1`) before any clustering; matched drafts inherit `epic_analog` by construction. Catalogs are versioned config-as-data. Cross-discipline links live in an explicit **ANALOGY map** artifact (typed: identical / partial / discarded_in_target / novel_in_target). G0/E0 unclassified buckets are first-class with a 5% alarm threshold. The 5+1-agent pipeline (Theme Discovery → Epic Extractor → Story Extractor → Validator → Cross-SOP Synthesis → Validator → Dependency Resolver) is unchanged in shape.
...
- ✅ **v3.4 spec drafted** (2026-05-05, `V3_4_SPEC.md` + `V3_4_DECISIONS_LOG.md`) — Static catalogs for tests + personas (D20); SME-free operation via four primitives: multi-agent quorum (D21, replaces SME ratification), auto-park (D22, replaces escalation), prior-discipline-as-oracle (D23, replaces curation), tightened thresholds + drift report (D24, replaces alarm review). Plus intake scope-check (D25 — later removed in v3.5).
- ✅ **v3.5 spec drafted** (2026-05-05/06, `V3_5_SPEC.md`) — Generalization reframe. Decisions D26 (POC scope = generalization), D27 (universal `lab_stage_v1`), D28 (persona `actor_type`), D29 (lift Theme Discovery deferral), D30 (no-HITL claim precision), D31 (structural enum extensions versioned), D32 (Pass 3b discard quorum), D33 (M=4 → M=3 vote threshold), D34 (prior-discipline narrowed; exemplars optional; holdout calibration dropped — supersedes D23), D35 (intake scope-check removed — supersedes D25).
- ✅ **v3.5 canonical doc shipped** (2026-05-05/06, `microbio_lims_architecture_v35.md`) — ~900-line clean-canonical reference. Discipline-agnostic framing (no Micro-POC scoping). 8 schemas with both JSON/YAML examples + per-field tables (Story, Epic, Theme catalog, Epic catalog, ANALOGY map, Persona catalog, Test catalog, `lab_stage_v1`). Two worked examples: Cyto → Microbiology (full Theme Discovery trace) + Cyto → Histology (showing same machinery on a different discipline). Glossary upfront with all key terms.
- ✅ **Three rounds of 5-pass parallel QC** completed on v3.5 (2026-05-05/06): round 1 found ~30 issues, round 2 found 2 stale `m_threshold: 4` instances, round 3 found 6 stale references introduced by D34/D35 edits — all fixed. Cyto-as-oracle → prior-discipline-as-oracle rename complete. No clean-canonical leaks. All schemas have field tables.
- ✅ **Native-PowerPoint client alignment deck** (2026-05-06, `client_alignment_deck.pptx` + `build_client_deck.py`) — 8 slides, 16:9, native shapes (boxes / arrows / decision diamonds / tables), no ASCII art. Slides: (1) what we are building, (2) what "prior discipline" means (themes + epics mandatory; exemplars optional), (3) the agents — cast of characters, (4) full flow at a glance, (5) Phase 1 flowchart + theme catalog YAML example, (6) Phase 2 flowchart + Story schema example, (7) Phase 3 flowchart + capability story example, (8) what ships + alignment ask. Color-coded: blue=process, yellow=agent/decision, green=output, pink=park. Footer with deck label + slide number.
...
| **v3.1** | Same agent pipeline + 4 story shapes + cross-SOP synthesis | Multi-shape Stories + capability stories from synthesis + per-culture configuration profile | Teaching corpus, with shape mix as a learned dimension |
| **v3.2** | + pre-flight Theme Discovery (warm-start conditioned discovery) + conditioned-mode Epic Extractor + versioned config-as-data catalogs + ANALOGY map + G0/E0 first-class | Same as v3.1, plus Output C (catalogs + ANALOGY map) | Source for prior themes + prior epics + exemplars + holdout |
| **v3.4** | + static catalogs (test, persona) + SME-free operation (quorum, auto-park, prior-as-oracle, drift report) + intake scope-check | Same as v3.2 | Same as v3.2 |
| **v3.5 (current)** | + generalization framing (any clinical discipline, not just Micro) + universal `lab_stage_v1` enum + persona `actor_type` + lift Theme Discovery deferral + simplifications (D34: prior narrowed to themes+epics, exemplars optional, holdout dropped; D35: intake scope-check removed) + quorum tuning (M=4 → M=3) + Pass 3b discard quorum | Same as v3.2, but per-discipline | Themes + epics mandatory; story exemplars optional; tasks unused |
